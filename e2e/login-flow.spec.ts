...
test.describe('Login Flow', () => {
  test('should show "Authentication not configured" when Supabase is missing', async ({ page }) => {
  ...
  test('should navigate to dashboard via "Continue without account"', async ({ page }) => {
    await gotoAndWaitForApp(page, '/login');
    await page.getByRole('button', { name: 'Continue without account' }).click();
    await expect(page).toHaveURL('/', { timeout: 10000 });
  });

  test('should allow normal navigation after continuing without account', async ({ page }) => {
    await gotoAndWaitForApp(page, '/login');
    await page.getByRole('button', { name: 'Continue without account' }).click();
    await expect(page).toHaveURL('/', { timeout: 10000 });
