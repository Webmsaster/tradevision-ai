...
export async function createTestTrade(page: Page, pair: string = "TEST/USDT") {
  ...
  // R8 Task G: relative date (7 days ago) so the test stays valid as the
  // wall-clock advances past 2026-01-10 — was hardcoded.
  const dateInputs = modal.locator('input[type="datetime-local"]');
  const day = new Date(Date.now() - 7 * 86400000).toISOString().slice(0, 10);
  await dateInputs.first().fill(`${day}T09:00`);
