...
export function detectRevengeTrade(trades: Trade[]): AIInsight | null {
  ...
  for (let i = 2; i < sorted.length; i++) {
    ...
    if (prev1.pnl < 0 && prev2.pnl < 0) {
      const prevAvgSize = (prev1.quantity * prev1.entryPrice + prev2.quantity * prev2.entryPrice) / 2;
      const currentSize = current.quantity * current.entryPrice;

      if (currentSize > prevAvgSize * 1.5) {
        const increasePercent = Math.round(((currentSize - prevAvgSize) / prevAvgSize) * 100);
...
export function detectHoldingLosers(trades: Trade[]): AIInsight | null {
  ...
  const avgWinHoldTime = winners.reduce((sum, t) => sum + getHoldTimeMs(t), 0) / winners.length;
  const avgLossHoldTime = losers.reduce((sum, t) => sum + getHoldTimeMs(t), 0) / losers.length;
  ...
  if (ratio > 2) {
    const avgWinHours = Math.round(avgWinHoldTime / (1000 * 60 * 60) * 10) / 10;
    const avgLossHours = Math.round(avgLossHoldTime / (1000 * 60 * 60) * 10) / 10;
...
export function detectTimePatterns(trades: Trade[]): AIInsight | null {
  ...
  const hourStats: Record<number, { total: number; wins: number; tradeIds: string[] }> = {};
...
export function detectOverleverageAfterWins(trades: Trade[]): AIInsight | null {
  ...
  for (let i = 3; i < sorted.length; i++) {
    ...
    if (avgStreakLeverage > 0 && nextLeverage > avgStreakLeverage * 1.5) {
      const increasePercent = Math.round(((nextLeverage - avgStreakLeverage) / avgStreakLeverage) * 100);
...
export function detectLossAversion(trades: Trade[]): AIInsight | null {
  ...
  if (ratio > 1.5) {
    return {
      ...
      severity: 8,
      relatedTrades: [
        ...losers.sort((a, b) => a.pnl - b.pnl).slice(0, 3).map((t) => t.id),
        ...winners.sort((a, b) => b.pnl - a.pnl).slice(0, 3).map((t) => t.id),
      ],
      category: 'loss-aversion',
    };
  }
...
export function detectConsistentPair(trades: Trade[]): AIInsight | null {
  ...
  const pairStats: Record<string, { total: number; wins: number; totalPnl: number; tradeIds: string[] }> = {};
...
export function detectGoodRiskManagement(trades: Trade[]): AIInsight | null {
  ...
  const maxLossPercent = totalEquity > 0 ? (maxSingleLoss / totalEquity) * 100 : 0;
...
export function detectOvertrading(trades: Trade[]): AIInsight | null {
  ...
  const dayStats: Record<string, { total: number; wins: number; tradeIds: string[] }> = {};
...
export function detectWeekendTrading(trades: Trade[]): AIInsight | null {
  ...
  const weekendWinRate = weekendTrades.filter((t) => t.pnl > 0).length / weekendTrades.length;
  const weekdayWinRate = weekdayTrades.filter((t) => t.pnl > 0).length / weekdayTrades.length;
...
export function detectImprovingPerformance(trades: Trade[]): AIInsight | null {
  ...
  if (improvement > 0.1) {
    ...
    return {
      ...
      description:
        `Your recent performance is improving! Win rate went from ${firstPercent}% to ${secondPercent}%.`,
      severity: 3,
