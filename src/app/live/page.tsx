...
// R-Perf: lazy-load recharts surface (~120KB) — chart is below-the-fold
// and signals + thesis render server-friendly without it.
const LiveChart = dynamic(
  () => import("./_LiveChart").then((m) => ({ default: m.LiveChart })),
        ...
        <div className="glass-card live-chart-card">
          ...
          <div style={{ width: "100%", height: 300 }}>
            {chartData.length > 0 ? (
              <LiveChart
                chartData={chartData}
                supports={analysis?.keyLevels.supports ?? []}
                resistances={analysis?.keyLevels.resistances ?? []}
                vwapNow={analysis?.vwapNow ?? null}
              />
            ) : (
              <div
                style={{
