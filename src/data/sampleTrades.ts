...
//
// R29-Frontend-Audit Bug 9: the raw sample rows below historically baked
// `* leverage` into the pnl/pnlPercent literals — diverging from the real
// `calculatePnl()` formula in `src/utils/calculations.ts` (which does NOT
// multiply by leverage). The exported `sampleTrades` now re-computes pnl
// and pnlPercent from each row's price/quantity/leverage/fees via the
// canonical formula so demo stats match the real engine.
// =============================================================================

const sampleTradesRaw: Trade[] = [
...
// R29-Frontend-Audit Bug 9: recompute pnl/pnlPercent from the canonical
// `calculatePnl()` formula. The raw literals above were leverage-multiplied
// (a copy-paste residue from an earlier engine spec) — exporting them
// directly would mis-state demo win-rate, P&L distribution and Sharpe.
export const sampleTrades: Trade[] = sampleTradesRaw.map((t) => {
  const { pnl, pnlPercent } = calculatePnl(t);
  return {
    ...t,
