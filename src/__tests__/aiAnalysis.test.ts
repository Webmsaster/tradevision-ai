...
describe('detectRevengeTrade', () => {
  ...
  it('detects revenge trading pattern', () => {
    const trades = [
      makeTrade({ pnl: -10, quantity: 1, entryPrice: 100, exitDate: '2024-01-01T00:00:00Z' }),
      makeTrade({ pnl: -15, quantity: 1, entryPrice: 100, exitDate: '2024-01-02T00:00:00Z' }),
      makeTrade({ pnl: 5, quantity: 2, entryPrice: 100, exitDate: '2024-01-03T00:00:00Z' }),
  ...
  it('returns null when position size does not increase', () => {
    const trades = [
      makeTrade({ pnl: -10, quantity: 1, entryPrice: 100, exitDate: '2024-01-01T00:00:00Z' }),
      makeTrade({ pnl: -15, quantity: 1, entryPrice: 100, exitDate: '2024-01-02T00:00:00Z' }),
      makeTrade({ pnl: 5, quantity: 1, entryPrice: 100, exitDate: '2024-01-03T00:00:00Z' }),
    ];
...
describe('detectHoldingLosers', () => {
  ...
  it('detects when losers are held much longer than winners', () => {
    const trades = [
      makeTrade({ pnl: 10, entryDate: '2024-01-01T10:00:00Z', exitDate: '2024-01-01T11:00:00Z' }), // 1h
      makeTrade({ pnl: 20, entryDate: '2024-01-02T10:00:00Z', exitDate: '2024-01-02T11:00:00Z' }), // 1h
      makeTrade({ pnl: -5, entryDate: '2024-01-03T10:00:00Z', exitDate: '2024-01-03T14:00:00Z' }),  // 4h
      makeTrade({ pnl: -10, entryDate: '2024-01-04T10:00:00Z', exitDate: '2024-01-04T14:00:00Z' }), // 4h
    ];
...
describe('detectOverleverageAfterWins', () => {
  it('returns null for fewer than 4 trades', () => {
    expect(detectOverleverageAfterWins([makeTrade(), makeTrade(), makeTrade()])).toBeNull();
  });
...
describe('detectImprovingPerformance', () => {
  it('detects improving win rate', () => {
    const trades = [
      // First half: 1 win, 2 losses = 33%
      makeTrade({ pnl: 10, exitDate: '2024-01-01T00:00:00Z' }),
      makeTrade({ pnl: -5, exitDate: '2024-01-02T00:00:00Z' }),
      makeTrade({ pnl: -5, exitDate: '2024-01-03T00:00:00Z' }),
      // Second half: 3 wins = 100%
      makeTrade({ pnl: 10, exitDate: '2024-01-04T00:00:00Z' }),
      makeTrade({ pnl: 10, exitDate: '2024-01-05T00:00:00Z' }),
      makeTrade({ pnl: 10, exitDate: '2024-01-06T00:00:00Z' }),
    ];
    const result = detectImprovingPerformance(trades);
    expect(result).not.toBeNull();
    expect(result!.type).toBe('positive');
  });
});
...
describe('detectDecliningPerformance', () => {
  it('detects declining win rate', () => {
    const trades = [
      // First half: 3 wins = 100%
      makeTrade({ pnl: 10, exitDate: '2024-01-01T00:00:00Z' }),
      makeTrade({ pnl: 10, exitDate: '2024-01-02T00:00:00Z' }),
      makeTrade({ pnl: 10, exitDate: '2024-01-03T00:00:00Z' }),
      // Second half: 1 win, 2 losses = 33%
      makeTrade({ pnl: 10, exitDate: '2024-01-04T00:00:00Z' }),
      makeTrade({ pnl: -5, exitDate: '2024-01-05T00:00:00Z' }),
      makeTrade({ pnl: -5, exitDate: '2024-01-06T00:00:00Z' }),
    ];
    const result = detectDecliningPerformance(trades);
    expect(result).not.toBeNull();
...
describe('detectPairSwitching', () => {
  it('returns null for fewer than 10 trades', () => {
    expect(detectPairSwitching(Array.from({ length: 5 }, () => makeTrade()))).toBeNull();
  });
...
describe('generateAllInsights', () => {
  ...
  it('returns insights sorted by severity descending', () => {
    ...
    const trades = [
      makeTrade({ pnl: 100, exitDate: '2024-01-01T00:00:00Z', pair: 'BTC/USDT' }),
      makeTrade({ pnl: -20, exitDate: '2024-01-02T00:00:00Z', pair: 'BTC/USDT' }),
      makeTrade({ pnl: -5, exitDate: '2024-01-03T00:00:00Z', pair: 'BTC/USDT' }),
      makeTrade({ pnl: -5, exitDate: '2024-01-04T00:00:00Z', pair: 'BTC/USDT' }),
      makeTrade({ pnl: -5, exitDate: '2024-01-05T00:00:00Z', pair: 'BTC/USDT' }),
      makeTrade({ pnl: 5, exitDate: '2024-01-06T00:00:00Z', pair: 'BTC/USDT' }),
      makeTrade({ pnl: -5, exitDate: '2024-01-07T00:00:00Z', pair: 'BTC/USDT' }),
    ];
