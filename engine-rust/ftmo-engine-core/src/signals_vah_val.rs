...
pub struct VahValParams { pub window: usize, pub n_buckets: usize, pub va_pct: f64 }
impl Default for VahValParams {
    fn default() -> Self { Self { window: 200, n_buckets: 50, va_pct: 0.70 } }
}
...
pub fn compute_vah_val_vote(candles: &[Candle], p: &VahValParams) -> Option<PositionSide> {
    ...
    let lows: Vec<f64> = bars.iter().filter_map(|c| if c.low.is_finite() { Some(c.low) } else { None }).collect();
    let highs: Vec<f64> = bars.iter().filter_map(|c| if c.high.is_finite() { Some(c.high) } else { None }).collect();
    ...
    for c in &bars {
        if !c.close.is_finite() || !c.volume.is_finite() { continue; }
    ...
    let mut sorted: Vec<(usize, f64)> = bucket_vol.iter().enumerate().map(|(i, v)| (i, *v)).collect();
    ...
    for (idx, vol) in &sorted {
        included.insert(*idx);
        acc += vol;
        if acc >= total_vol * p.va_pct { break; }
    }
    ...
    if close_now > vah { Some(PositionSide::Long) }
    else if close_now < val { Some(PositionSide::Short) }
    else { None }
}
