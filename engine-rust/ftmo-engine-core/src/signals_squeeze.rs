...
pub fn compute_squeeze_vote(candles: &[Candle], p: &SqueezeParams) -> Option<PositionSide> {
    if p.period < 2 || candles.len() < p.period + 4 {
        return None;
    }
    let i = candles.len() - 2;
    if i < p.period + 2 {
    ...
    if in_squeeze {
        return None;
    } // hold, no vote during squeeze
    let close_now = candles[i].close;
    if close_now > kc_upper {
        Some(PositionSide::Long)
    } else if close_now < kc_lower {
