...
fn haar_d1(arr: &[f64]) -> Vec<f64> {
    let n = arr.len() / 2;
    (0..n)
        .map(|i| (arr[2 * i] - arr[2 * i + 1]) / 2.0_f64.sqrt())
        .collect()
}
...
pub fn compute_wavelet_vote(candles: &[Candle], p: &WaveletParams) -> Option<PositionSide> {
    if p.window < 16 || p.window.count_ones() != 1 {
        return None;
    } // must be power of 2
    ...
    let lo = i + 1 - p.window;
    let closes: Vec<f64> = (lo..=i)
        .map(|j| candles[j].close)
        .filter(|c| c.is_finite())
        .collect();
    if closes.len() != p.window {
        return None;
    }
    ...
    if energy_ratio > p.d1_energy_max {
        return None;
    } // chop, skip
      // Trending — vote on a3 direction
    if a3.last() > a3.first() {
        Some(PositionSide::Long)
    } else if a3.last() < a3.first() {
