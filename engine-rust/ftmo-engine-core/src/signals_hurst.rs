...
pub fn compute_hurst_vote(candles: &[Candle], p: &HurstParams) -> Option<PositionSide> {
    if p.window < 20 || candles.len() < p.window + p.slope_lookback + 3 {
        return None;
    }
    let i = candles.len() - 2;
    if i < p.window + p.slope_lookback {
        return None;
    }
    let lo = i + 1 - p.window;
    let returns: Vec<f64> = (lo + 1..=i)
        .filter_map(|j| {
            let a = candles[j - 1].close;
            let b = candles[j].close;
