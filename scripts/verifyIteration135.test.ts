/**
 * Iter 135 — 5-gate battery for ATR-adaptive TP winner.
 *
 * iter134 winner: fixed 1% stop, tp2 = 8 × ATR(14), everything else iter133.
 *   in-sample (2083d): n=2498, tpd 1.20, WR 58.2%, mean 0.035%, Sharpe 10.15,
 *   cumRet +136.3%, bs+ 100%, bs5% +77.1%, pctProf 90%, minW -0.8%
 *
 * Gates (STRICTEST tier — knowing iter133 already passed):
 *   G1 full: tpd ≥ 1.1, Sharpe ≥ 9, bs+ ≥ 98%, pctProf ≥ 85%, ret > 0
 *   G2 quarters: all 4 positive
 *   G3 tpMult ∈ {6, 7, 8, 9, 10}: all Sharpe ≥ 7
 *   G4 sensitivity: 10 variants, ≥ 80% pass Sharpe ≥ 6 & mean ≥ 0.02%
 *   G5 OOS 60/40: Sharpe ≥ 6, mean ≥ 0.02%, bs+ ≥ 90%
 */
...
function executeLong(
  ...
  const tp2L = entry + p.tpAtrMult * atrI;
...
function run(candles: Candle[], ctx: Ctx, p: Params): Trade[] {
  ...
  for (let i = p.macroBars + 2; i < candles.length - 1; i++) {
    ...
    for (const m of mechs) {
      ...
      const r = executeLong(candles, ctx, i, p);
      if (!r) continue;
...
describe("iter 135 — ATR-TP final 5-gate", () => {
  it("full 5-gate battery on tp atr 8×", { timeout: 1_500_000 }, async () => {
    console.log("\n=== ITER 135: FINAL LOCK (ATR tp 8×) ===");
    ...
    console.log(`loaded ${c.length} BTC 1h candles (${days.toFixed(0)} days)`);
    ...
    // G3 tpAtrMult sweep
    console.log("G3 tpAtrMult sweep:");
    let g3 = true;
    for (const mult of [6, 7, 8, 9, 10]) {
      const p = { ...BASE, tpAtrMult: mult };
      const t = run(c, ctx, p);
      const ss = stats(t, days, bpw, 300 + mult);
      if (ss.sh < 7) g3 = false;
      console.log(
        `  atrMult=${mult} n=${ss.n} mean=${(ss.mean * 100).toFixed(3)}% Shp=${ss.sh.toFixed(2)} bs+=${(ss.bs.pctPositive * 100).toFixed(0)}%`,
      );
    }
    ...
    const vs: Array<{ label: string; p: Params }> = [
      ...
      { label: "rsi-5", p: { ...BASE, rsiTh: 37 } },
      { label: "rsi+5", p: { ...BASE, rsiTh: 47 } },
      { label: "nHi=24", p: { ...BASE, nHi: 24 } },
      { label: "volMult=1.0", p: { ...BASE, volMult: 1.0 } },
    ];
    let vPass = 0;
    for (const v of vs) {
      ...
      const ok = ss.sh >= 6 && ss.mean >= 0.0002 && ss.ret > 0;
      if (ok) vPass++;
      console.log(
        `  ${v.label.padEnd(12)} n=${ss.n.toString().padStart(4)} mean=${(ss.mean * 100).toFixed(3).padStart(6)}% Shp=${ss.sh.toFixed(2).padStart(5)} ${ok ? "★" : ""}`,
      );
    }
    ...
    const g5 =
      oosS.sh >= 6 && oosS.mean >= 0.0002 && oosS.bs.pctPositive >= 0.9;

    console.log(
      `\n── VERDICT ──\nG1=${g1 ? "✓" : "✗"} G2=${g2 ? "✓" : "✗"} G3=${g3 ? "✓" : "✗"} G4=${g4 ? "✓" : "✗"} G5=${g5 ? "✓" : "✗"}  ${g1 && g2 && g3 && g4 && g5 ? "★★★ ALL GATES PASS ★★★" : "— fails"}`,
    );
  });
});
