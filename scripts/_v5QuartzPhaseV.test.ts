/**
 * Phase V — per-asset stopPct on V5_QUARTZ.
 * V5_QUARTZ has different engine stack than V5_TITANIUM (Phase Q tested on
 * TITANIUM). With atrStop p56m2 + chand + breakEven 3% the SL behavior is
 * different — per-asset SP may have a new optimum.
 */
...
const LOG_FILE = `${LOG_DIR}/V5_QUARTZ_PHASE_V_${STAMP}.log`;
...
describe("V5_QUARTZ Phase V", { timeout: 6 * 3600_000 }, () => {
  it("greedy per-asset stopPct on QUARTZ 30m", async () => {
    mkdirSync(LOG_DIR, { recursive: true });
    writeFileSync(
      LOG_FILE,
      `V5_QUARTZ_PHASE_V START ${new Date().toISOString()}\n`,
    );
    ...
    const SPs = [0.025, 0.03, 0.035, 0.04, 0.045, 0.05];
    ...
    for (const asset of best.assets.map((a) => a.symbol)) {
      ...
      log(
        `  → ${asset} best sp=${(aBestSp * 100).toFixed(1)}% (running 3d=${(bestR.pass3 * 100).toFixed(2)}%)`,
      );
    }
    ...
    writeFileSync(
      `${LOG_DIR}/V5_QUARTZ_PHASE_V_${STAMP}.json`,
      JSON.stringify(
        {
          baseline: baseR,
