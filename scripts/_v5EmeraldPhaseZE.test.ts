/**
 * Phase ZE — asset add round-4 on V5_EMERALD (18 assets).
 * Try the remaining FTMO crypto candidates.
 */
...
const LOG_FILE = `${LOG_DIR}/V5_EMERALD_PHASE_ZE_${STAMP}.log`;
...
describe("V5_EMERALD Phase ZE", { timeout: 8 * 3600_000 }, () => {
  it("greedy add round-4", async () => {
    mkdirSync(LOG_DIR, { recursive: true });
    writeFileSync(
      LOG_FILE,
      `V5_EMERALD_PHASE_ZE START ${new Date().toISOString()}\n`,
    );

    const baseSyms = syms(FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_EMERALD);
    const allSyms = [
    ...
    let stacked = FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_EMERALD;
    let stackedR = baseR;
    ...
    log(
      `vs V5_EMERALD: 1d=+${((stackedR.pass1 - baseR.pass1) * 100).toFixed(2)}pp / 3d=+${((stackedR.pass3 - baseR.pass3) * 100).toFixed(2)}pp`,
    );

    writeFileSync(
      `${LOG_DIR}/V5_EMERALD_PHASE_ZE_${STAMP}.json`,
      JSON.stringify(
        {
          baseline: baseR,
