/**
 * Phase Q — per-asset stopPct retune on V5_TITANIUM 30m.
 * Phase I on 2h V5_PLATINUM showed sp=5% optimal for all assets, but 30m may
 * differ — smaller bars, different volatility regime per asset.
 */
...
const LOG_FILE = `${LOG_DIR}/V5_TITANIUM_PHASE_Q_${STAMP}.log`;
...
describe("V5_TITANIUM Phase Q", { timeout: 6 * 3600_000 }, () => {
  it("greedy per-asset stopPct on 30m", async () => {
    mkdirSync(LOG_DIR, { recursive: true });
    writeFileSync(
      LOG_FILE,
      `V5_TITANIUM_PHASE_Q START ${new Date().toISOString()}\n`,
    );

    const symbols = syms(FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_TITANIUM);
    ...
    const SPs = [0.025, 0.03, 0.035, 0.04, 0.045, 0.05]; // bounded by liveCaps maxStopPct=0.05
    let best = FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_TITANIUM;
    let bestR = baseR;
    ...
    for (const asset of best.assets.map((a) => a.symbol)) {
      ...
      log(
        `  → ${asset} best sp=${(aBestSp * 100).toFixed(1)}% (running 1d=${(bestR.pass1 * 100).toFixed(2)}%)`,
      );
    }
    ...
    writeFileSync(
      `${LOG_DIR}/V5_TITANIUM_PHASE_Q_${STAMP}.json`,
      JSON.stringify(
        {
          baseline: baseR,
