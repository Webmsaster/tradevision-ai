...
async function main() {
  ...
  try {
    ...
    await runStep("Login page + continue without account", async () => {
      ...
      await page
        .getByRole("heading", { name: /TradeVision AI/i })
        .first()
        .waitFor({
          state: "visible",
          timeout: 20000,
      ...
      const continueBtn = page.getByRole("button", {
        name: /Continue without account/i,
      });
    ...
    await runStep("Trades page + add trade", async () => {
      ...
      await page
        .getByRole("button", { name: "Add Trade", exact: true })
        .click();
      // The trades table normalizes pair symbols (ETH/USDT -> ETHUSDT), so
