/**
 * Phase ZF — per-asset TP retune on V5_PEARL (focus on ATOM).
 */
...
const LOG_FILE = `${LOG_DIR}/V5_PEARL_PHASE_ZF_${STAMP}.log`;
...
describe("V5_PEARL Phase ZF", { timeout: 8 * 3600_000 }, () => {
  it("per-asset TP retune", async () => {
    mkdirSync(LOG_DIR, { recursive: true });
    writeFileSync(
      LOG_FILE,
      `V5_PEARL_PHASE_ZF START ${new Date().toISOString()}\n`,
    );

    const symbols = syms(FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_PEARL);
    ...
    let best = FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_PEARL;
    let bestR = baseR;
    ...
    writeFileSync(
      `${LOG_DIR}/V5_PEARL_PHASE_ZF_${STAMP}.json`,
      JSON.stringify(
        {
          baseline: baseR,
