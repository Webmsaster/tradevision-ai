/**
 * Phase ZA — per-asset TP retune on V5_TOPAZ (post-RUNE-drop).
 */
...
const LOG_FILE = `${LOG_DIR}/V5_TOPAZ_PHASE_ZA_${STAMP}.log`;
...
describe("V5_TOPAZ Phase ZA", { timeout: 6 * 3600_000 }, () => {
  it("per-asset TP retune", async () => {
    mkdirSync(LOG_DIR, { recursive: true });
    writeFileSync(
      LOG_FILE,
      `V5_TOPAZ_PHASE_ZA START ${new Date().toISOString()}\n`,
    );
    ...
    let best = FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_TOPAZ;
    let bestR = baseR;
    ...
    writeFileSync(
      `${LOG_DIR}/V5_TOPAZ_PHASE_ZA_${STAMP}.json`,
      JSON.stringify(
        {
          baseline: baseR,
