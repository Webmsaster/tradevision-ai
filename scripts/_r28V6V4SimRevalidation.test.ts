/**
 * R28_V6 V4-Engine Re-validation (Round 58 Agent 3/6).
 *
 * Round 56 added two engine fixes that could shift R28_V6 pass-rate:
 *   1. Funding-cost is now DEDUCTED from PnL (was filter-only).
 *   2. atrStop look-ahead fix + volMult-aware lossFloor.
 * Round 57 added:
 *   3. V4 Day-30 force-close at last bar (positions held to end-of-window
 *      now realised at last bar close).
 *   4. kellyPnls inline trim, challengePeak defensive init.
 *
 * Pre-R56 baseline (from `r28v5_tp_finegrid.log` 2026-05-02): 60.29% / med 4d
 * across 136 windows on uniform tpMult=0.55 (= R28_V6 config).
 *
 * This test re-runs ONLY R28_V6 against the latest engine and reports:
 *   pass-rate / med / p90 / TL-fail / DL-fail / time-fail / give-back-fail.
 *
 * Methodology mirrors `_r28V5TpFineGrid.test.ts` exactly:
...
describe("R28_V6 V4-Engine Re-validation", { timeout: 30 * 60_000 }, () => {
  it("re-measures R28_V6 pass-rate on post-R56/R57 engine", () => {
    ...
    plog("\n=== R28_V6 V4-ENGINE RE-VALIDATION (post-R56/R57) ===");
    plog(
      `pass-rate:           ${r.rate.toFixed(2)}% (${r.passes}/${r.windows})`,
    );
    ...
    const totalReasons = Object.values(r.reasonCounts).reduce(
      (a, b) => a + b,
      0,
    );
    ...
    plog(`post-R56/R57 rate:   ${r.rate.toFixed(2)}%`);
    plog(`drift:               ${drift >= 0 ? "+" : ""}${drift.toFixed(2)}pp`);
    if (Math.abs(drift) <= 0.74) {
      plog(
        "interpretation:      within ±1 window noise — fixes confirmed neutral",
      );
    } else if (drift > 0.74) {
      plog(
        "interpretation:      MATERIAL POSITIVE drift — engine fix improved real edge",
      );
    } else if (drift > -2) {
      plog(
        "interpretation:      minor regression — likely Day-30 force-close realising small open losses",
      );
    } else {
      plog(
        "interpretation:      >2pp regression — investigate funding/atrStop/Day-30",
      );
    }
  });
});
