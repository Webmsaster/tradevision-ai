/**
 * Phase ZC — asset add round-3 on V5_RUBIN.
 * V5_RUBIN has slightly different engine than V5_TOPAZ (INJ tp 5.0%); maybe
 * MATIC/UNI/MANA/STX/MKR — previously tied — now help.
 */
...
const LOG_FILE = `${LOG_DIR}/V5_RUBIN_PHASE_ZC_${STAMP}.log`;
...
describe("V5_RUBIN Phase ZC", { timeout: 8 * 3600_000 }, () => {
  it("greedy add on V5_RUBIN", async () => {
    mkdirSync(LOG_DIR, { recursive: true });
    writeFileSync(
      LOG_FILE,
      `V5_RUBIN_PHASE_ZC START ${new Date().toISOString()}\n`,
    );

    const baseSyms = syms(FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_RUBIN);
    const allSyms = [
    ...
    let stacked = FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_RUBIN;
    let stackedR = baseR;
    ...
    log(
      `vs V5_RUBIN: 1d=+${((stackedR.pass1 - baseR.pass1) * 100).toFixed(2)}pp / 3d=+${((stackedR.pass3 - baseR.pass3) * 100).toFixed(2)}pp`,
    );

    writeFileSync(
      `${LOG_DIR}/V5_RUBIN_PHASE_ZC_${STAMP}.json`,
      JSON.stringify(
        {
          baseline: baseR,
