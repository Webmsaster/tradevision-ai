/**
 * Push V5_QUARTZ Step 1 (8%/30d) above 55%.
 */
...
describe("V5_QUARTZ Step 1 push to 55", { timeout: 60 * 60_000 }, () => {
  it("variations on Step 1 mode", async () => {
    ...
    const T = FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_TOPAZ;
    const A = FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_AMBER;
    const Z = FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_ZIRKON;
    const top4 = (cfg: FtmoDaytrade24hConfig) =>
      cfg.assets.filter((a) =>
        ["ETH-TREND", "BTC-TREND", "BNB-TREND", "BCH-TREND"].includes(a.symbol),
      );
    ...
    const cases: Array<{ name: string; cfg: FtmoDaytrade24hConfig }> = [
      { name: "QUARTZ baseline (Step 1)", cfg: Q },
      { name: "QUARTZ top-4", cfg: { ...Q, assets: top4(Q) } },
      { name: "QUARTZ top-6", cfg: { ...Q, assets: top6(Q) } },
      { name: "QUARTZ lev=3", cfg: { ...Q, leverage: 3 } },
      {
        name: "QUARTZ lev=3 + top-4",
        cfg: { ...Q, leverage: 3, assets: top4(Q) },
      },
      {
        name: "QUARTZ lev=3 + maxRiskFrac=0.3",
        cfg: {
          ...Q,
          leverage: 3,
      ...
      {
        name: "QUARTZ tp 7%→10%",
        cfg: { ...Q, assets: Q.assets.map((a) => ({ ...a, tpPct: 0.1 })) },
      },
      {
        name: "QUARTZ holdBars 240→480",
        cfg: {
          ...Q,
          holdBars: 480,
          assets: Q.assets.map((a) => ({ ...a, holdBars: 480 })),
        },
      },
      { name: "QUARTZ + minTrad=2", cfg: { ...Q, minTradingDays: 2 } },
      { name: "TOPAZ baseline", cfg: T },
      { name: "TOPAZ top-4", cfg: { ...T, assets: top4(T) } },
      { name: "AMBER baseline", cfg: A },
      { name: "AMBER top-4", cfg: { ...A, assets: top4(A) } },
      { name: "ZIRKON baseline", cfg: Z },
      { name: "ZIRKON top-4", cfg: { ...Z, assets: top4(Z) } },
      { name: "ZIRKON lev=3", cfg: { ...Z, leverage: 3 } },
    ];
