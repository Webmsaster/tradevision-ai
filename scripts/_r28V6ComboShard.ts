...
for (let start = WARMUP; start + winBars <= minBars; start += stepBars) {
  ...
  const r = simulate(trimmed, cfg, WARMUP, WARMUP + winBars, "R28_V6_COMBO");
  appendFileSync(
    OUT_FILE,
    JSON.stringify({
      winIdx,
  ...
  console.log(
    `[combo ${SHARD_IDX}/${SHARD_COUNT}] win=${winIdx} passed=${r.passed} reason=${r.reason} eq=${r.finalEquityPct.toFixed(4)} t+${Math.round((Date.now() - t0) / 1000)}s`,
  );
  winIdx++;
}
console.log(
  `[combo ${SHARD_IDX}/${SHARD_COUNT}] DONE in ${Math.round((Date.now() - t0) / 1000)}s`,
);
