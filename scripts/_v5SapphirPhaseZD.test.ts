/**
 * Phase ZD — per-asset TP retune on V5_SAPPHIR (18 assets).
 */
...
const LOG_FILE = `${LOG_DIR}/V5_SAPPHIR_PHASE_ZD_${STAMP}.log`;
...
describe("V5_SAPPHIR Phase ZD", { timeout: 8 * 3600_000 }, () => {
  it("per-asset TP retune", async () => {
    mkdirSync(LOG_DIR, { recursive: true });
    writeFileSync(
      LOG_FILE,
      `V5_SAPPHIR_PHASE_ZD START ${new Date().toISOString()}\n`,
    );

    const symbols = syms(FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_SAPPHIR);
    ...
    let best = FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_SAPPHIR;
    let bestR = baseR;
    ...
    writeFileSync(
      `${LOG_DIR}/V5_SAPPHIR_PHASE_ZD_${STAMP}.json`,
      JSON.stringify(
        {
          baseline: baseR,
