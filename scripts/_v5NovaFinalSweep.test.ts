/**
 * Final V5_NOVA push-to-55 — combined optimizations + Step 2 mode test.
 */
...
describe("V5_NOVA FINAL push-to-55", { timeout: 60 * 60_000 }, () => {
  it("combined optimizations + Step 2 mode", async () => {
    ...
    const top4 = base.assets.filter((a) =>
      ["ETH-TREND", "BTC-TREND", "BNB-TREND", "BCH-TREND"].includes(a.symbol),
    );

    const cases: Array<{ name: string; cfg: FtmoDaytrade24hConfig }> = [
      { name: "NOVA baseline (8% target / 30d)", cfg: base },
      // STEP 2 MODE: 5% target / 60 days
      {
        name: "NOVA Step2 mode (5%/60d)",
        cfg: { ...base, profitTarget: 0.05, maxDays: 60 },
      },
      {
        name: "NOVA Step2 + top-4 assets",
        cfg: { ...base, profitTarget: 0.05, maxDays: 60, assets: top4 },
      },
      {
        name: "NOVA Step2 + lev=3",
        cfg: { ...base, profitTarget: 0.05, maxDays: 60, leverage: 3 },
      },
      {
        name: "NOVA Step2 + lev=3 + top-4",
        cfg: {
          ...base,
          profitTarget: 0.05,
          maxDays: 60,
      ...
      // 8% target combined
      {
        name: "NOVA top-4 + tp=10%",
        cfg: {
          ...base,
          tpPct: 0.1,
          assets: top4.map((a) => ({ ...a, tpPct: 0.1 })),
        },
      },
      {
        name: "NOVA top-4 + lev=3 + tp=10%",
        cfg: {
          ...base,
          leverage: 3,
          tpPct: 0.1,
      ...
      {
        name: "NOVA top-4 + lev=3 + maxRiskFrac=0.3",
        cfg: {
          ...base,
          leverage: 3,
      ...
      // Lower minTradingDays
      { name: "NOVA + minTrad=2", cfg: { ...base, minTradingDays: 2 } },
      {
        name: "NOVA + minTrad=2 + top-4",
        cfg: { ...base, minTradingDays: 2, assets: top4 },
      },
    ];
