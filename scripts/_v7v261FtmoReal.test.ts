/**
 * Final honest test: V7/V261 with REAL FTMO Step 1 target (8%, not 10%) +
 * liveCaps. This is the truthful production setting.
 */
...
describe("FTMO REAL configurations", { timeout: 30 * 60_000 }, () => {
  it("V7/V261 across realistic FTMO settings", async () => {
    ...
    const cases = [
      // V261 - target=8% (real FTMO) + liveCaps
      {
        name: "V261 t=8% + liveCaps",
        cfg: {
          ...FTMO_DAYTRADE_24H_CONFIG_V261_2H_OPT,
          liveCaps,
          profitTarget: 0.08,
      ...
      // V261 - higher maxRiskFrac (e.g., 0.6 = 6% per stop)
      {
        name: "V261 t=8% + maxRiskFrac=0.6",
        cfg: {
          ...FTMO_DAYTRADE_24H_CONFIG_V261_2H_OPT,
    ...
    console.log(
      `\n${"variant".padEnd(32)} 3d-pass    wr      med   p90  TL%   DL%   trades`,
    );
    for (const c of cases) {
      ...
      console.log(
        `${c.name.padEnd(32)} ${(r.passRate * 100).toFixed(2).padStart(7)}% ${(r.wr * 100).toFixed(1).padStart(5)}%  ${String(r.med).padStart(2)}d  ${String(r.p90).padStart(2)}d  ${tlPct.padStart(5)}% ${dlPct.padStart(5)}%  ${r.avgTradesPerWindow.toFixed(2).padStart(5)}${star}`,
      );
    }
