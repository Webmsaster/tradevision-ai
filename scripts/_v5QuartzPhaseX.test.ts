/**
 * Phase X — GA third-pass on V5_QUARTZ. Different seed, broader sweep.
 */
...
const LOG_FILE = `${LOG_DIR}/V5_QUARTZ_PHASE_X_${STAMP}.log`;
const TRIALS = 100;
...
function buildVariant(rand: () => number): {
  ...
  if (rand() < 0.3) {
    ...
    cfg.assets = cfg.assets.map((a) => ({
      ...a,
      tpPct: Math.max(0.01, (a.tpPct ?? 0.02) + tpShift),
    }));
  }
  return { cfg, meta };
}

describe("V5_QUARTZ Phase X", { timeout: 6 * 3600_000 }, () => {
  it("GA third-pass random search seed=20260501", async () => {
    mkdirSync(LOG_DIR, { recursive: true });
    writeFileSync(
      LOG_FILE,
      `V5_QUARTZ_PHASE_X START ${new Date().toISOString()}\n`,
    );
    ...
    writeFileSync(
      `${LOG_DIR}/V5_QUARTZ_PHASE_X_${STAMP}.json`,
