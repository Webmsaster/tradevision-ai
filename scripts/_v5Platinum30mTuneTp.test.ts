/**
 * Phase O — per-asset TP retune on V5_PLATINUM_30M.
 * V5_PLATINUM TPs were optimized on 2h. On 30m, optimum may differ.
 */
...
const LOG_FILE = `${LOG_DIR}/V5_PLAT_30M_TP_${STAMP}.log`;
...
describe("V5_PLATINUM_30M Phase O", { timeout: 8 * 3600_000 }, () => {
  it("per-asset TP retune on 30m", async () => {
    mkdirSync(LOG_DIR, { recursive: true });
    writeFileSync(
      LOG_FILE,
      `V5_PLAT_30M_TP START ${new Date().toISOString()}\n`,
    );

    const symbols = syms(FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_PLATINUM_30M);
    ...
    const baseR = evaluate(
      "V5_PLATINUM_30M baseline",
      FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_PLATINUM_30M,
      data,
    );
    log(fmt(baseR));

    const TPs = [0.025, 0.03, 0.035, 0.04, 0.045, 0.05, 0.055, 0.06];
    let best = FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_PLATINUM_30M;
    let bestR = baseR;
    ...
    for (const asset of best.assets.map((a) => a.symbol)) {
      ...
      for (const tp of TPs) {
        ...
        log(fmt(r));
        // Optimize for step=1d (robustness)
    ...
    writeFileSync(
      `${LOG_DIR}/V5_PLAT_30M_TP_${STAMP}.json`,
      JSON.stringify(
        {
          baseline: baseR,
