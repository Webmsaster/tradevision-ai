/**
 * Phase F — Per-asset TP fine-tune on V5_DIAMOND (14 assets).
 *
 * V5_GOLD tuned TP per-asset on V5_PRO (10 assets); V5_DIAMOND added 4 more
 * assets which still have default TP=4%. Greedy single-axis tune those + retune
 * the original 10 in-context.
 */
...
const LOG_FILE = `${LOG_DIR}/V5_DIAMOND_PHASE_F_${STAMP}.log`;
...
describe("V5_DIAMOND Phase F", { timeout: 6 * 3600_000 }, () => {
  it("per-asset TP tune on V5_DIAMOND (14 assets)", async () => {
    mkdirSync(LOG_DIR, { recursive: true });
    writeFileSync(
      LOG_FILE,
      `V5_DIAMOND_PHASE_F START ${new Date().toISOString()}\n`,
    );

    const symbols = syms(FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_DIAMOND);
    ...
    let best = FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_DIAMOND;
    let bestR = baseR;
    ...
    for (const asset of best.assets.map((a) => a.symbol)) {
      log(`\n========== ${asset} ==========`);
      let aBest = bestR;
      let aBestTp = best.assets.find((a) => a.symbol === asset)?.tpPct ?? 0.04;
      for (const tp of TPs) {
      ...
      log(
        `  → ${asset} best tp=${(aBestTp * 100).toFixed(1)}% (running pass ${(bestR.passRate * 100).toFixed(2)}%)`,
      );
    }
    ...
    log(
      `vs V5_DIAMOND baseline: ${(bestR.passRate * 100).toFixed(2)}% vs ${(baseR.passRate * 100).toFixed(2)}% = +${((bestR.passRate - baseR.passRate) * 100).toFixed(2)}pp`,
    );

    writeFileSync(
      `${LOG_DIR}/V5_DIAMOND_PHASE_F_${STAMP}.json`,
      JSON.stringify(
        {
          baseline: baseR,
