/**
 * Phase ZI — per-asset TP retune on V5_JADE (20 assets).
 */
...
const LOG_FILE = `${LOG_DIR}/V5_JADE_PHASE_ZI_${STAMP}.log`;
...
describe("V5_JADE Phase ZI", { timeout: 8 * 3600_000 }, () => {
  it("per-asset TP retune", async () => {
    mkdirSync(LOG_DIR, { recursive: true });
    writeFileSync(
      LOG_FILE,
      `V5_JADE_PHASE_ZI START ${new Date().toISOString()}\n`,
    );

    const symbols = syms(FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_JADE);
    ...
    const TPs = [0.015, 0.02, 0.025, 0.03, 0.035, 0.04, 0.045, 0.05];
    let best = FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_JADE;
    let bestR = baseR;
    ...
    for (const asset of best.assets.map((a) => a.symbol)) {
      ...
      log(
        `  → ${asset} best tp=${(aBestTp * 100).toFixed(1)}% (running 3d=${(bestR.pass3 * 100).toFixed(2)}%)`,
      );
    }
    ...
    writeFileSync(
      `${LOG_DIR}/V5_JADE_PHASE_ZI_${STAMP}.json`,
      JSON.stringify(
        {
          baseline: baseR,
