/**
 * R46 — 3000-trial random search anchored on V5_TITAN_REAL
 * Goal: find configs with score > 40.52% via random combinatorial sampling
 */
...
const LOG_FILE = `${LOG_DIR}/R46_${new Date().toISOString().replace(/[:.]/g, "-")}.log`;
...
describe("R46 — 3000-trial random search", { timeout: 24 * 3600_000 }, () => {
  it("runs R46", async () => {
    mkdirSync(LOG_DIR, { recursive: true });
    writeFileSync(LOG_FILE, `R46 START ${new Date().toISOString()}\n`);
    ...
    log(`Base: score=${(baseR.score * 100).toFixed(2)}%`);
    ...
    for (let trial = 0; trial < 3000; trial++) {
      ...
      // Per-asset tp variants
      const useTpVariant = Math.random() < 0.3;
      const tpMap: Record<string, number> = useTpVariant
        ? {
            "BTC-TREND": pickv([0.05, 0.06, 0.07]),
            "ETH-TREND": pickv([0.06, 0.07, 0.08]),
            "BNB-TREND": pickv([0.06, 0.07, 0.08, 0.1]),
            "ADA-TREND": pickv([0.06, 0.07, 0.08]),
            "DOGE-TREND": pickv([0.07, 0.08, 0.1, 0.12]),
            "LTC-TREND": pickv([0.06, 0.07, 0.08]),
            "BCH-TREND": pickv([0.06, 0.07, 0.08, 0.1]),
            "LINK-TREND": pickv([0.06, 0.07, 0.08, 0.1]),
          }
        : {};

      const useHbVariant = Math.random() < 0.3;
      const hbMap: Record<string, number> = useHbVariant
        ? {
            "BTC-TREND": pickv([120, 180, 240]),
            "ETH-TREND": pickv([180, 240, 360]),
            "DOGE-TREND": pickv([120, 240, 360]),
          }
        : {};
      ...
      const cfg: FtmoDaytrade24hConfig = {
        ...
        crossAssetFiltersExtra: maybe(0.5, () => [
          {
            symbol: pickv(["ETHUSDT", "BTCUSDT", "BNBUSDT", "ADAUSDT"]),
        ...
        assets: FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_TITAN_REAL.assets.map(
          (a) => ({
            ...a,
            tpPct: tpMap[a.symbol] ?? a.tpPct,
            holdBars: hbMap[a.symbol] ?? a.holdBars,
