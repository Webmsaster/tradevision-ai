/**
 * Step 1 push via RISK REDUCTION instead of leverage boost.
 * Goal: cut DL fails (40-43%) without losing pass-rate.
 */
...
describe("V5_QUARTZ Step 1 RISK-REDUCE push", { timeout: 60 * 60_000 }, () => {
  it("Lower DL fails via tighter risk caps + cooldown tweaks", async () => {
    ...
    const cases: Array<{ name: string; cfg: FtmoDaytrade24hConfig }> = [
      { name: "QUARTZ top-6 lev=2 baseline", cfg: { ...Q, assets: top6(Q) } },
      {
        name: "QUARTZ top-6 + maxRisk=0.30",
        cfg: {
          ...Q,
      ...
      {
        name: "QUARTZ top-6 + maxRisk=0.25",
        cfg: {
          ...Q,
      ...
      {
        name: "QUARTZ top-6 + maxRisk=0.20",
        cfg: {
          ...Q,
          liveCaps: { maxStopPct: 0.05, maxRiskFrac: 0.2 },
          assets: top6(Q),
        },
      },
      {
        name: "QUARTZ top-6 + maxStop=0.04",
        cfg: {
          ...Q,
          liveCaps: { maxStopPct: 0.04, maxRiskFrac: 0.4 },
          assets: top6(Q),
        },
      },
      {
        name: "QUARTZ top-6 + maxStop=0.03",
        cfg: {
          ...Q,
      ...
      {
        name: "QUARTZ top-6 + LSC after=1 cd=100",
        cfg: {
          ...Q,
          lossStreakCooldown: { afterLosses: 1, cooldownBars: 100 },
          assets: top6(Q),
        },
      },
      {
        name: "QUARTZ top-6 + maxConcurrentTrades=2",
        cfg: { ...Q, maxConcurrentTrades: 2, assets: top6(Q) },
      },
      {
        name: "QUARTZ top-6 + maxConcurrentTrades=1",
        cfg: { ...Q, maxConcurrentTrades: 1, assets: top6(Q) },
      },
      {
        name: "QUARTZ top-6 maxRisk=0.3 + MCT=2",
        cfg: {
          ...Q,
          liveCaps: { maxStopPct: 0.05, maxRiskFrac: 0.3 },
      ...
      // Combine reduce + tp boost
      {
        name: "QUARTZ top-6 maxRisk=0.3 + tp=10%",
        cfg: {
          ...Q,
          liveCaps: { maxStopPct: 0.05, maxRiskFrac: 0.3 },
          assets: top6(Q).map((a) => ({ ...a, tpPct: 0.1 })),
        },
      },
    ];
