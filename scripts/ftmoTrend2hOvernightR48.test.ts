/**
 * R48 — re-validate ALL volTgt configs with honest engine + TL-aware search
 *
 * Score function: mean - 0.5*std - 2*TL_rate (penalize catastrophe)
 */
...
const LOG_FILE = `${LOG_DIR}/R48_${new Date().toISOString().replace(/[:.]/g, "-")}.log`;
...
describe(
  "R48 — honest re-validation + TL-aware search",
  { timeout: 24 * 3600_000 },
  () => {
    it("runs R48", async () => {
      mkdirSync(LOG_DIR, { recursive: true });
      writeFileSync(LOG_FILE, `R48 START ${new Date().toISOString()}\n`);
      ...
      function evalCfg(cfg: FtmoDaytrade24hConfig) {
        ...
        // TL-aware score: penalize catastrophe
        const score = mean - 0.5 * std - 2.0 * meanTL;
        return { mean, min, std, recent3, meanTL, score };
      }

      log(
        `========== HONEST RE-VAL of all V5 configs (engine fixed) ==========`,
      );
      log(
        `${"Config".padEnd(20)} mean    min     std    recent3 TLrate  score`,
      );
      const order = [
        ...
        ["V5_TITAN_REAL", FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_TITAN_REAL],
        ["V5_NOVA", FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_NOVA],
      ] as const;
      let baseR = { score: -1 } as ReturnType<typeof evalCfg>;
      let baseN = "V5";
      for (const [name, cfg] of order) {
        const r = evalCfg(cfg);
        log(
          `${name.padEnd(20)} ${(r.mean * 100).toFixed(2)}%  ${(r.min * 100).toFixed(2)}%  ${(r.std * 100).toFixed(2)}%  ${(r.recent3 * 100).toFixed(2)}%  ${(r.meanTL * 100).toFixed(2)}%   ${(r.score * 100).toFixed(2)}%`,
        );
      ...
      log(`\nBest by TL-aware score: ${baseN}`);

      log(
        `\n========== TL-aware random search 1500 trials anchored on V5 ==========`,
      );
      let best = {
        name: "V5",
        cfg: FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5,
        ...evalCfg(FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5),
      };
      log(`V5 baseline TL-score: ${(best.score * 100).toFixed(2)}%`);
      ...
      for (let trial = 0; trial < 1500; trial++) {
        ...
        const cfg: FtmoDaytrade24hConfig = {
          ...
          trailingStop: {
            ...
            trailPct: pickv([0.001, 0.003, 0.005]),
          },
          momentumRanking: maybe(0.5, () => ({
        ...
        if ((trial + 1) % 250 === 0) {
          log(
            `  ${trial + 1}/1500 — best score=${(best.score * 100).toFixed(2)}%`,
          );
        }
      }
      ...
      if (best.score > evalCfg(FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5).score) {
        writeFileSync(
          `${LOG_DIR}/R48_BEST.json`,
