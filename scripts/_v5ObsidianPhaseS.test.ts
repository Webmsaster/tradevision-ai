/**
 * Phase S — GA random search 200 trials on V5_OBSIDIAN.
 * Combines all engine + asset dimensions for final pass-rate push.
 */
...
const LOG_FILE = `${LOG_DIR}/V5_OBSIDIAN_PHASE_S_${STAMP}.log`;
const TRIALS = 150;
...
function buildVariant(rand: () => number): {
  ...
  let cfg = structuredClone(FTMO_DAYTRADE_24H_CONFIG_TREND_2H_V5_OBSIDIAN);
  ...
  if (rand() < 0.3) {
    ...
    meta.htf = `${lb}/${apply}/${cfg.htfTrendFilter.threshold}`;
  }
  if (rand() < 0.3) {
    cfg.chandelierExit = {
      period: pick(rand, [14, 28, 56]),
  ...
  if (rand() < 0.25) {
    cfg.breakEven = { threshold: pick(rand, [0.02, 0.025, 0.03]) };
    meta.be = cfg.breakEven.threshold;
  }
  if (rand() < 0.3) {
    cfg.maxConcurrentTrades = pick(rand, [4, 6, 8, 10, 12]);
...
describe("V5_OBSIDIAN Phase S", { timeout: 8 * 3600_000 }, () => {
  it("GA random search", async () => {
    mkdirSync(LOG_DIR, { recursive: true });
    writeFileSync(
      LOG_FILE,
      `V5_OBSIDIAN_PHASE_S START ${new Date().toISOString()}\n`,
    );
    ...
    writeFileSync(
      `${LOG_DIR}/V5_OBSIDIAN_PHASE_S_${STAMP}.json`,
