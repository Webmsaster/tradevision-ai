...
- **Strategy/FTMO tests** (`scripts/ftmo*.test.ts` and `scripts/exploratory/`): Heavy backtests run via vitest.
  ...
  - **🏆 Active Single-Account Champion (post-R29 audit, 2026-05-08): `R28_V6_PASSLOCK`** — 55.88% honest post-R9. V12-hunt chase aborted; 65% single-account target NOT achieved. Multi-account 3-strategy (~91% min-1-pass) is the only path to 65%+ live. See "Prev Champion" line below for full details.
