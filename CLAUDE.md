...
- **Strategy/FTMO tests** (`scripts/ftmo*.test.ts` and `scripts/exploratory/`): Heavy backtests run via vitest.
  - **🏆 Active Single-Account Champion (2026-05-08): `R28_V6_PASSLOCK`** (`FTMO_TF=2h-trend-v5-r28-v6-passlock`). **Honest pass-rate 55.88%** (76/136 windows, post-R9 gap-trade fix `46d9bb3`). Bootstrap 95% CI `[47.79%, 63.97%]` — old 63.24% claim was engine-bug-inflated tail-event. Conservative live ~50-58% single-account. Multi-account 3-Strategy (PASSLOCK+TITANIUM+AMBER) ≈ **91% min-1-pass**. Mechanism: `closeAllOnTargetReached` flag eliminates give_back (0% in MC). Live deploy: `tools/PASSLOCK_DEPLOY_RUNBOOK.md`. Detail in `MEMORY.md`.
